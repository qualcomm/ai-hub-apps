...
if [ ! -f "$APK" ]; then
    echo "::error::APK not found at $APK for semantic_segmentation_android. Build it first." >&2
    exit 1
fi
if [ "$RUN_TEST" -eq 1 ] && [ ! -f "$TEST_APK" ]; then
    echo "::error::Test APK not found at $TEST_APK for semantic_segmentation_android. Build it first." >&2
    exit 1
fi
...
if [ "$RUN_TEST" -eq 1 ]; then
    ...
    echo "::step::Running instrumentation tests for semantic_segmentation_android"
    set +e
    ...
    if [ "$RC" -ne 0 ] || echo "$OUTPUT" | grep -qE "INSTRUMENTATION_FAILED|FAILURES"; then
        echo "::error::Instrumentation tests failed for semantic_segmentation_android." >&2
        exit 1
    fi
    echo "::done::test"
else
    echo "::step::Launching semantic_segmentation_android ($PACKAGE)"
