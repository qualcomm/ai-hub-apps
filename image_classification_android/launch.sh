...
if [ "$RUN_TEST" -eq 1 ]; then
    ...
    echo "::step::Running instrumentation tests for image_classification_android"
    set +e
    ...
    if [ "$RC" -ne 0 ] || echo "$OUTPUT" | grep -qE "INSTRUMENTATION_FAILED|FAILURES"; then
        echo "::error::Instrumentation tests failed for image_classification_android." >&2
        exit 1
    fi
    echo "::done::test"
else
    echo "::step::Launching image_classification_android ($PACKAGE)"
