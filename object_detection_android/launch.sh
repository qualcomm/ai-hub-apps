...
if [ ! -f "$APK" ]; then
    echo "::error::APK not found at $APK for object_detection_android. Build it first." >&2
    exit 1
fi
if [ "$RUN_TEST" -eq 1 ] && [ ! -f "$TEST_APK" ]; then
    echo "::error::Test APK not found at $TEST_APK for object_detection_android. Build it first." >&2
    exit 1
fi
...
if [ "$RUN_TEST" -eq 1 ]; then
    ...
    echo "::step::Running instrumentation tests for object_detection_android"
    set +e
    ...
    if [ "$RC" -ne 0 ] || echo "$OUTPUT" | grep -qE "INSTRUMENTATION_FAILED|FAILURES"; then
        echo "::error::Instrumentation tests failed for object_detection_android." >&2
        exit 1
    fi
    echo "::done::test"
else
    echo "::step::Launching object_detection_android ($PACKAGE)"
