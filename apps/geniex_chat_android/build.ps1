...
if (-not (Test-Path "$AppDir\Dockerfile")) {
    Write-Host "::error::No Dockerfile found for geniex_chat_android; it cannot be built."
    exit 1
}
...
if (-not (Test-Path "$AppDir\scripts")) {
    Write-Host "::error::No scripts\ directory found for geniex_chat_android. Docker builds need a bundled app; use 'qai-hub-apps fetch geniex_chat_android'."
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "::error::Failed to build the image for geniex_chat_android. If '$BaseImage' could not be pulled, check network access to it, or set QAIHA_BASE_IMAGE to a base image you built locally."
    exit 1
}
