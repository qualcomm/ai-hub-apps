...
if (-not (Test-Path "$AppDir\Dockerfile")) {
    Write-Error "::error::No Dockerfile found for super_resolution_windows_cpp. Re-run with -NoDocker to build natively."
    exit 1
}
...
if (-not (Test-Path "$AppDir\scripts")) {
    Write-Error "::error::No scripts\ directory found for super_resolution_windows_cpp. Docker builds need a bundled app; use 'qai-hub-apps fetch super_resolution_windows_cpp'."
    exit 1
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error "::error::Failed to build the image for super_resolution_windows_cpp. If '$BaseImage' could not be pulled, check network access to it, or set QAIHA_BASE_IMAGE to a base image you built locally from tools/docker/windows.dockerfile. Alternatively re-run with -NoDocker to build natively."
        exit 1
    }
