...
if (-not (Test-Path "$AppDir\Dockerfile")) {
    Write-Error "::error::No Dockerfile found for chatapp_windows_cpp. Re-run with -NoDocker to build natively."
    exit 1
}

if (-not (Test-Path "$AppDir\scripts")) {
    Write-Error "::error::No scripts\ directory found for chatapp_windows_cpp. Docker builds need a bundled app; use 'qai-hub-apps fetch chatapp_windows_cpp'."
    exit 1
}
