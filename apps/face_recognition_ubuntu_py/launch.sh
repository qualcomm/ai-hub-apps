...
if ! $SUDO docker build --build-arg "BASE_IMAGE=$BASE_IMAGE" -t "$IMAGE_TAG" .; then
    echo "::error::Failed to build the image for face_recognition_ubuntu_py. If '$BASE_IMAGE' could not be pulled, check network access to it, or set QAIHA_BASE_IMAGE to a base image you built locally from tools/docker/ubuntu.dockerfile. Alternatively re-run with --no-docker to run natively." >&2
    exit 1
fi
