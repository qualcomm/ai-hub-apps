...
$Runner = "com.quicinc.imageclassification.test/androidx.test.runner.AndroidJUnitRunner"
...
if (-not (Test-Path $Apk)) {
    Write-Host "::error::APK not found at $Apk for image_classification_android. Build it first."
    exit $QaihaExitBuildRequired
}
if ($Test -and -not (Test-Path $TestApk)) {
    Write-Host "::error::Test APK not found at $TestApk for image_classification_android. Build it first."
    exit $QaihaExitBuildRequired
}
...
if ($Test) {
    ...
    Write-Host "::step::Running instrumentation tests for image_classification_android"
    $Output = (adb -s $Serial shell am instrument -w -r $Runner 2>&1) -join "`n"
    $Rc = $LASTEXITCODE
    Write-Host $Output
    if ($Rc -ne 0 -or $Output -match "INSTRUMENTATION_FAILED|FAILURES") {
        Write-Host "::error::Instrumentation tests failed for image_classification_android."
        exit 1
    }
    Write-Host "::done::test"
} else {
    Write-Host "::step::Launching image_classification_android ($Package)"
