...
if [ ! -f "$APP_DIR/Dockerfile" ]; then
    echo "::error::No Dockerfile found for chatapp_android; it cannot be built." >&2
    exit 1
fi
...
if [ ! -d "$APP_DIR/scripts" ]; then
    echo "::error::No scripts/ directory found for chatapp_android. Docker builds need a bundled app; use 'qai-hub-apps fetch chatapp_android'." >&2
    exit 1
fi
