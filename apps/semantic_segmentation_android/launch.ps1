...
$Runner = "com.quicinc.semanticsegmentation.test/androidx.test.runner.AndroidJUnitRunner"
...
if (-not (Test-Path $Apk)) {
    Write-Host "::error::APK not found at $Apk for semantic_segmentation_android. Build it first."
    exit $QaihaExitBuildRequired
}
if ($Test -and -not (Test-Path $TestApk)) {
    Write-Host "::error::Test APK not found at $TestApk for semantic_segmentation_android. Build it first."
    exit $QaihaExitBuildRequired
}
...
if ($Test) {
    ...
    Write-Host "::step::Running instrumentation tests for semantic_segmentation_android"
    $Output = (adb -s $Serial shell am instrument -w -r $Runner 2>&1) -join "`n"
    $Rc = $LASTEXITCODE
    Write-Host $Output
    if ($Rc -ne 0 -or $Output -match "INSTRUMENTATION_FAILED|FAILURES") {
        Write-Host "::error::Instrumentation tests failed for semantic_segmentation_android."
        exit 1
    }
    Write-Host "::done::test"
} else {
    Write-Host "::step::Launching semantic_segmentation_android ($Package)"
