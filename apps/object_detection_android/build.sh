...
if [ ! -f "$APP_DIR/Dockerfile" ]; then
    echo "::error::No Dockerfile found for object_detection_android; it cannot be built." >&2
    exit 1
fi
...
if [ ! -d "$APP_DIR/scripts" ]; then
    echo "::error::No scripts/ directory found for object_detection_android. Docker builds need a bundled app; use 'qai-hub-apps fetch object_detection_android'." >&2
    exit 1
fi
...
if ! docker build "${build_args[@]}" -t "$IMAGE_TAG" .; then
    echo "::error::Failed to build the image for object_detection_android. If '$BASE_IMAGE' could not be pulled, check network access to it, or set QAIHA_BASE_IMAGE to a base image you built locally." >&2
    exit 1
fi
