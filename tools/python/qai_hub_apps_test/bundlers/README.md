...
2. Inside a temp directory, runs three steps in order:
   ...
   - **`bundle_scripts`** — processes `install_*.sh` / `install_*.ps1`,
     `test.sh` / `test.ps1`, the hand-written `run.sh` / `run.ps1`, and the
     generated `build.sh` / `build.ps1` and `launch.sh` / `launch.ps1` (see
     [Building Apps](../../../../CONTRIBUTING.md#building-apps) and
...
- `test.sh` / `test.ps1` and `run.sh` / `run.ps1` (hand-written) and `build.sh` /
  `build.ps1` and `launch.sh` / `launch.ps1` (generated) are processed the same way

> `build.*` and `launch.*` are **not** hand-written — they are produced by
> `generate_app_scripts` (see [Building Apps](../../../../CONTRIBUTING.md#building-apps)
> and [Running Apps](../../../../CONTRIBUTING.md#running-apps)). The bundler treats
> them like any other script: it rewrites their `source` lines and pulls in the shared
